...
$(document).on('click','#fast-com',function(){
  ...
  var rContent = $('#textcomment').val();
    ...
    type:'post',
      url:'/addComment',
      data:{'pid':pid,'ownerid':uid,'content':rContent},
      dataType:'json',
      success:function(msg){
        ...
        $this.text('发布评论');
        $this.removeAttr('disabled');
        num = parseInt($('#pinglun_num').text());
        num = num+1;
        $('#pinglun_num').text(num);
        $('#ping-num').text(num);
...
$(document).on('click','#rep-comment',function(){
  ...
  var rContent = $('#textarea_inner').val();
    ...
    type:'post',
      url:'/repComment',
      data:{'pid':pid,'ownerid':uid,'commid':comid,'replayid':replayid,'content':rContent},
      dataType:'json',
      success:function(msg){
...
$(document).on('click','#report-submit',function(){
  ...
  $.ajax({
    type:'post',
          url:'/reportComment',
          data:{'cid':comid,'tid':examine_type,'content':contentVal},
          dataType:'json',
