window.onload = function(){
	 ...
	 for(var i = 4;i < shopData.length;i++){
	 	strShop += '<div class="commodity-items" id="'+shopData[i].id+'">'+
					'<img src="" data-img="' + shopData[i].src[0] + '" alt="">'+
					'<div class="title">' + shopData[i].title + '</div>'+
					'<div class="price clear">'+
						'<span>￥<em>' + shopData[i].price + '</em></span><b>' + shopData[i].sales + '人付款</b>'+
					'</div>'+
	...
	/* 1-5  商品分类数据跳转 */
	 ...
	 for(var i = 0;i<typeList.length;i++){
		...
		jump(typeList[4],'/ui/tbmobile/category');  //添加分类页面查看入口
	 }

	/* 1-6  跳转到搜索 */
	var goSearch = document.getElementById('goSearch');
	jump(goSearch,'/ui/tbmobile/search');
	...
	jump(click,'/ui/tbmobile/news');

	/*底部页面跳转*/
	var foot = document.getElementsByTagName('footer')[0];
	var divs = foot.getElementsByTagName('div');
	jump(divs[0],'/ui/tbmobile/index');
	jump(divs[1],'/ui/tbmobile/news');
	jump(divs[2],'/ui/tbmobile/shopcar');
	jump(divs[3],'/ui/tbmobile/user');
